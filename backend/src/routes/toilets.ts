...
router.get('/', async (req, res) => {
  try {
    ...
    const toilets = await getQuery<Toilet>(
      `SELECT * FROM toilets ${whereClause} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
    
    // Get total count for pagination
    const totalCount = await getSingle<{ count: number }>(
      `SELECT COUNT(*) as count FROM toilets ${whereClause}`,
      params
    );
  ...
  try {
    ...
    const result = await runQuery(
      `INSERT INTO toilets (name, address, latitude, longitude, type, price, description, approved)
       VALUES (?, ?, ?, ?, ?, ?, ?, 0)`,
      [name, address, latitude, longitude, type, price || null, description || null]
    );
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const toilet = await getSingle<Toilet>(
      'SELECT * FROM toilets WHERE id = ? AND approved = 1',
      [id]
    );
